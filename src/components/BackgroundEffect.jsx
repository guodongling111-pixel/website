...
`;
    ...
    resizeObserverRef.current = new ResizeObserver(() => {
      ...
      resizeRafRef.current = requestAnimationFrame(() => manager.resize());
    });
    ...
    intersectionObserverRef.current = new IntersectionObserver(([entry]) => {
      ...
      if (isVisibleRef.current) manager.loop();
      else manager.pause();
    });
    ...
    // 5. 组件卸载时的清理逻辑
    return () => {
      if (webglRef.current) webglRef.current.destroy();
      if (resizeObserverRef.current) resizeObserverRef.current.disconnect();
      if (intersectionObserverRef.current) intersectionObserverRef.current.disconnect();
      if (resizeRafRef.current) cancelAnimationFrame(resizeRafRef.current);
    };
  }, [
    autoDemo, autoSpeed, autoIntensity, autoResumeDelay, 
    autoRampDuration, takeoverDuration, colors, mouseForce, 
    cursorSize, resolution, viscous, iterationsViscous, 
    iterationsPoisson, dt, BFECC, isBounce, isViscous
  ]);

  // 最终渲染结构
  return (
    <div className={`background-effect-wrapper ${className}`} style={style}>
